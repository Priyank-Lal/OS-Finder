        ...
        <div className="mb-8">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-1">
                <h1 className="text-3xl font-bold tracking-tight">
                  {repo.owner} / <span className="text-primary">{repo.repo_name}</span>
                </h1>
          ...
          <div className="flex flex-wrap gap-3 items-center">
             {repo.ai_categories?.map((cat: string) => (
                <Badge key={cat} variant="secondary" className="capitalize px-2 py-1">
                  {cat}
                </Badge>
              ))}
              ...
              <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
                ...
                <span className="font-medium text-foreground">{(repo.stars / 1000).toFixed(1)}k</span>
              </div>
              <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 ...
                 <Card className="border-l-4 border-l-green-500">
                    ...
                    <CardContent>
                      <ul className="space-y-3">
                        {repo.beginner_tasks?.length > 0 ? (
                          repo.beginner_tasks.map((task: any, i: number) => (
                             <li key={i} className="text-sm">
                                <p className="font-medium text-foreground mb-0.5">{task.title}</p>
                                <p className="text-muted-foreground text-xs">{task.why}</p>
                             </li>
                          ))
                        ) : (
                           <li className="text-sm text-muted-foreground">Check "Good First Issues" on GitHub.</li>
                        )}
                      </ul>
                    </CardContent>
                 </Card>
                 ...
                 <Card className="border-l-4 border-l-blue-500">
                    ...
                    <CardContent>
                       <ul className="space-y-3">
                        {repo.intermediate_tasks?.length > 0 ? (
                          repo.intermediate_tasks.map((task: any, i: number) => (
                           ...
                           <li className="text-sm text-muted-foreground">Look for "Help Wanted" issues.</li>
                        )}
                      </ul>
                    </CardContent>
                 </Card>
            ...
            <section>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
                  <IssueCard
                    ...
                    count={repo.issue_data?.good_first_issue_count || 0}
                    description="Start here"
                    color="green"
                  />
                  <IssueCard
                    title="Help Wanted"
                    count={repo.issue_data?.help_wanted_count || 0}
                    description="Community help"
                    color="blue"
                  />
                  <IssueCard
                    title="Bugs"
                    count={repo.issue_data?.bug_count || 0}
                  ...
                  <CardContent className="space-y-4">
                    {repo.issue_samples.map((issue: any, i: number) => (
                      <div key={i} className="flex items-start gap-3 pb-4 border-b border-border last:border-0 last:pb-0">
                        ...
                        <div>
                          <p className="font-medium text-sm mb-1 line-clamp-1">{issue.title}</p>
                          <div className="flex flex-wrap gap-2">
                            {issue.labels?.slice(0, 3).map((label: string) => (
                              <Badge key={label} variant="outline" className="text-[10px] h-5 px-1.5">
                                {label}
                              </Badge>
                            ))}
                            {issue.created_at && (
          ...
          <div className="lg:col-span-4 space-y-6">
             ...
             <Card className="border-primary/20 shadow-sm">
               <CardHeader className="pb-4">
                 <CardTitle className="flex items-center justify-between">
                   OS-Finder Score
                   <span className="text-2xl font-bold text-primary">{repo.overall_score || 0}</span>
                 </CardTitle>
                 <CardDescription>AI-evaluated contribution readiness</CardDescription>
               </CardHeader>
             ...
             <ActivityStats
                ...
                lastCommit={repo.last_commit ? formatDistanceToNow(new Date(repo.last_commit), { addSuffix: true }) : "N/A"}
              />
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                   <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium truncate max-w-[150px]" title={repo.licenseInfo?.name}>{repo.licenseInfo?.name || "None"}</span>
                   </div>
                   <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Code of Conduct</span>
                      <span className="font-medium">{repo.community_health?.has_code_of_conduct ? "Yes" : "No"}</span>
                   </div>
                   <div className="flex justify-between text-sm">
                      <span className="text-muted-foreground">Contributing Guide</span>
                      <span className="font-medium">{repo.community_health?.has_contributing ? "Yes" : "No"}</span>
                   </div>
                   <div className="pt-2 border-t border-border/50 flex justify-between text-xs text-muted-foreground">
                      <span>Updated</span>
                      <span>{repo.last_updated ? new Date(repo.last_updated).toLocaleDateString() : "N/A"}</span>
                   </div>
                </CardContent>
              </Card>
